  ...
  class ProductsToolbar extends HTMLElement {
    ...
    init() {
      ...
      if (this.layoutSwitcher) {
        ...
        const layout = window.theme?.storageUtil?.get('products-layout') || localStorage.getItem('products-layout');
      ...
      if (this.compareToggle) {
        ...
        const isChecked = window.theme?.storageUtil?.get('compare-toggle', true) ?? localStorage.getItem('compare-toggle') === 'true';
        if (isChecked !== null) {
      ...
      } else if (window.theme?.settings?.compareToggle === 'none') {
    ...
    toggleCompare(evt, checked = null, persist = true) {
      ...
      if (persist && !Shopify.designMode) {
        if (window.theme?.storageUtil) {
          window.theme.storageUtil.set('compare-toggle', isChecked);
        } else {
          localStorage.setItem('compare-toggle', isChecked);
        }
      }
    ...
    toggleFilters() {
      ...
      if (window.theme?.mediaMatches?.lg || window.matchMedia('(min-width: 1024px)').matches) {
    ...
    handleLayoutChange(evt, layout) {
      ...
      if (!layout) {
        if (window.theme?.storageUtil) {
          window.theme.storageUtil.set('products-layout', evt.target.value);
        } else {
          localStorage.setItem('products-layout', evt.target.value);
        }
      }
    }
