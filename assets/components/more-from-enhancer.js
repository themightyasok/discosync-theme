...
if (!customElements.get('more-from-enhancer')) {
  class MoreFromEnhancer extends HTMLElement {
    ...
    createDebugPanel() {
      ...
      let existingPanel = document.getElementById('grouping-debug-panel');
      if (existingPanel) {
    ...
    async checkAndEnhance() {
      ...
      const wrappers = document.querySelectorAll('.dynamic-artist-collection-wrapper, .dynamic-label-collection-wrapper, .dynamic-genre-collection-wrapper');
    ...
    async enhanceSection(wrapper) {
      ...
      // Always use Storefront API for comprehensive results and proper grouping
        ...
        this.debugLog('🟢 MoreFromEnhancer: Using Storefront API');
          await this.fetchAndGroupViaStorefrontAPI(searchType, searchValue, currentProductId, wrapper);
        } else {
        ...
        await this.fetchViaSectionAPI(sectionId, searchType, searchValue, currentProductId, wrapper);
      }
    }
    
    async fetchAndGroupViaStorefrontAPI(searchType, searchValue, currentProductId, wrapper, append = false) {
      try {
        this.debugLog(`🟢 MoreFromEnhancer: Starting Storefront API fetch for ${searchType}: "${searchValue}"`);
        
        // Build search query for Storefront API
        let searchQuery = this.buildStorefrontSearchQuery(searchType, searchValue);
        ...
        while (hasNextPage && products.length < 200) { // Limit to 200 for "more from" sections
          ...
          try {
            ...
            this.debugLog(`🟢 MoreFromEnhancer: API response received`, { hasProducts: !!data.products, edgeCount: data.products?.edges?.length || 0 });
        ...
        this.debugLog(`🟢 MoreFromEnhancer: Created ${grouped.groups.length} groups and ${grouped.singles.length} singles`);
        ...
        const sectionId = wrapper.dataset.sectionId;
        await this.fetchViaSectionAPI(sectionId, searchType, searchValue, currentProductId, wrapper, append);
      }
    }
    ...
    async renderGroupedProducts(grouped, wrapper, append = false) {
      ...
      let container = wrapper.querySelector('ul.slider__grid, .products-grid-container ul, ul.grid');
      ...
      this.debugLog(`🔵 MoreFromEnhancer: Starting Section Rendering API calls for ${allItems.length} items`);
      ...
      const cardPromises = allItems.slice(0, 8).map(async (item, index) => {
        try {
          ...
          this.debugLog(`🔵 MoreFromEnhancer: Fetching card ${index + 1}/${Math.min(8, allItems.length)}`, {
    ...
    async fetchViaSectionAPI(sectionId, searchType, searchValue, currentProductId, wrapper, append = false) {
      try {
        ...
        const searchResponse = await fetch(`/search/suggest.json?q=${encodeURIComponent(searchQuery)}&resources[type]=product&resources[limit]=100`);
        const searchData = await searchResponse.json();
        
        if (!searchData.resources?.results?.products) return;
        ...
        const newContainer = sectionElement.querySelector('ul.slider__grid, .products-grid-container ul, ul.grid');
        const existingContainer = wrapper.querySelector('ul.slider__grid, .products-grid-container ul, ul.grid');
        
        if (newContainer && existingContainer) {
          await this.renderProducts(Array.from(newContainer.querySelectorAll('li')), wrapper, append, existingContainer);
        }
      } catch (error) {
    ...
    async renderProducts(newItems, wrapper, append, existingContainer) {
      if (!existingContainer) {
        existingContainer = wrapper.querySelector('ul.slider__grid, .products-grid-container ul, ul.grid');
      }
      ...
      if (append) {
        ...
        const existingHandles = new Set(
          Array.from(existingContainer.querySelectorAll('[data-product-handle], a[href*="/products/"]'))
            .map(el => {
        ...
        newItems.forEach(item => {
          const handle = item.querySelector('[data-product-handle]')?.dataset.productHandle ||
