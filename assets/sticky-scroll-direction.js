...
if (!customElements.get('sticky-scroll-direction')) {
  class StickyScrollDirection extends HTMLElement {
     ...
     */
    toggleListeners() {
      const mediaMatches = window.theme?.mediaMatches || {
        lg: window.matchMedia('(min-width: 1024px)').matches,
        md: window.matchMedia('(min-width: 768px)').matches
      };
      if ((this.minStickySize === 'lg' && mediaMatches.lg)
        || (this.minStickySize === 'md' && mediaMatches.md)) {
        this.addListeners();
      } else {
    ...
    handleDisclosureChange(evt) {
      if (evt.target.classList.contains('disclosure__panel')) {
        ...
        const elementUtil = window.theme?.elementUtil || {
          isInViewport: (elem) => {
            const rect = elem.getBoundingClientRect();
            return rect.top >= 0 && rect.left >= 0 && rect.bottom <= window.innerHeight && rect.right <= window.innerWidth;
          }
        };
        if (!elementUtil.isInViewport(summaryElem)) {
          window.scrollTo({
            behavior: 'smooth',
